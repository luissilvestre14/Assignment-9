...
# Your code here
sales_df = pd.read_csv(sales_data_csv, skipinitialspace=True)
...
# Your code here
promo_group = sales_df.groupby('Promotion')['Total_Sales']
promo_comparison = {
    'promo_avg_sales'       : promo_group.mean().get('Yes', np.nan),
    'no_promo_avg_sales'    : promo_group.mean().get('No',  np.nan),
    'promo_total_revenue'   : promo_group.sum().get('Yes',  np.nan),
    'no_promo_total_revenue': promo_group.sum().get('No',   np.nan),
}
...
# Your code here
_region_category = sales_df.groupby(['Region', 'Category'])['Total_Sales'].sum()
top_categories_by_region = _region_category.groupby(level='Region').idxmax().apply(lambda x: x[1])
...
# Your code here
_product_totals = sales_df.groupby('Product')['Total_Sales'].sum()
product_revenue_analysis = pd.DataFrame({
    'total_revenue': _product_totals,
    'percentage'   : (_product_totals / total_revenue * 100).round(2)
...
# Your code here
print("\nRegional Performance:")
for region, rev in sales_by_region.items():
...
print("1. North America leads in total revenue — increase inventory and targeted")
print("   promotions in this region to capitalize on its strong demand.")
print("2. Promoted items generate higher average sales; expanding the promotions")
print("   program across more products and regions could lift overall revenue.")
print("3. Missing data in 'Units' and 'Unit_Price' columns should be addressed")
print("   at the point of entry to ensure accurate reporting and forecasting.")